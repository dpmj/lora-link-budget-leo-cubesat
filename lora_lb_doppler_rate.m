...
% Warning: Use 0.03277 s as minimum, it will crash otherwise because it needs a lot of RAM
...
sampleTime = symbolperiod_s;  % [s] Simulation step (a small step is necessary for doppler rate calc)
...
pkt12_t = 9.02;  % [s] how long a packet of sf12 lasts, 255 bytes
pkt11_t = 5.00;  % [s] how long a packet of sf11 lasts
pkt10_t = 2.79;  % [s] how long a packet of sf10 lasts

pkt12_n = ceil(pkt12_t/sampleTime);  % how many samples does the packet last
pkt11_n = ceil(pkt11_t/sampleTime);
pkt10_n = ceil(pkt10_t/sampleTime);
...
for altitude=altitudes
    ...
    pkt12_ddiff_loop(loop_iter, :) = pkt12_ddiff;
    pkt11_ddiff_loop(loop_iter, :) = pkt11_ddiff;
    pkt10_ddiff_loop(loop_iter, :) = pkt10_ddiff;

    % -----------------------
    % doppler rate, new receivers (LR11xx, SX1302)

    % https://www.semtech.com/design-support/lora-calculator
    % 255 bytes, 125 khz, 868 MHz

    sym12_n = ceil(sym12_t/sampleTime);  % how many samples does the packet last

    sym12_ddiff = zeros(1, length(doppler_time) - sym12_n);  % alloc

    % calculate the doppler rate (substract samples pkt12_n samples apart)

    for i = 1:(length(doppler_time) - sym12_n)
        sym12_ddiff(i) = doppler_fshift(i+sym12_n) - doppler_fshift(i);
    end
    sym12_ddiff = abs(sym12_ddiff);

    sym12_ddiff_loop(loop_iter, :) = sym12_ddiff;
...
subplot(3, 2, 1);
plot(doppler_time(1:end-pkt12_n), pkt12_ddiff_loop);
hold on;
grid on; grid minor;
xlim([doppler_time(1), doppler_time(end)]);
title(sprintf("Doppler rate, SX12xx, SF12, 255 B, t_{pkg} = %1.2f s", pkt12_t));
xlabel("Sim. time");
xlim([dl_g dr_g]);
...
subplot(3, 2, 3);
plot(doppler_time(1:end-pkt11_n), pkt11_ddiff_loop);
hold on;
grid on; grid minor;
xlim([doppler_time(1), doppler_time(end)]);
title(sprintf("Doppler rate, SX12xx, SF11, 255 B, t_{pkg} = %1.2f s", pkt11_t));
xlabel("Sim. time");
xlim([dl_g dr_g]);
ylabel("Doppler rate (Hz/T_{pkt})");
ylim([0, pkt_ylim]);
...
subplot(3, 2, 5);
plot(doppler_time(1:end-pkt10_n), pkt10_ddiff_loop);
hold on;
grid on; grid minor;
xlim([doppler_time(1), doppler_time(end)]);
title(sprintf("Doppler rate, SX12xx, SF10, 255 B, t_{pkg} = %1.2f s", pkt10_t));
xlabel("Sim. time");
xlim([dl_g dr_g]);
ylabel("Doppler rate (Hz/T_{pkt})");
ylim([0, pkt_ylim]);
...
% ------------------------------------------------------------------------------
% doppler rate limits for LoRa LR11xx and SX1302, LDRO=on

sym12_lim = floor((0.1 * 125e3) / (2^12));
sym11_lim = floor((0.1 * 125e3) / (2^11));
sym10_lim = floor((0.1 * 125e3) / (2^10));
...
subplot(3, 2, 2);
plot(doppler_time(1:end-sym12_n), sym12_ddiff_loop);
hold on;
grid on; grid minor;
xlim([doppler_time(1), doppler_time(end)]);
title(sprintf("Doppler rate, LR11xx, SF12, t_{sym} = %1.4f s", sym12_t));
xlabel("Sim. time");
xlim([dl_g dr_g]);
ylabel("Doppler rate (Hz/T_{sym})");
ylim([0, sym_ylim]);
yticks(0:sym_tick:sym_ylim);
yline(sym12_lim, Color="#5e5e5e", LineStyle="--", Label=sprintf("%d Hz", sym12_lim), LabelHorizontalAlignment='left');
legend(orbits_labels, Location="northeast");

subplot(3, 2, 4);
plot(doppler_time(1:end-sym12_n), sym12_ddiff_loop./2);
hold on;
grid on; grid minor;
xlim([doppler_time(1), doppler_time(end)]);
title(sprintf("Doppler rate, LR11xx, SF11, t_{sym} = %1.4f s", sym11_t));
xlabel("Sim. time");
xlim([dl_g dr_g]);
ylabel("Doppler rate (Hz/T_{sym})");
ylim([0, sym_ylim]);
yticks(0:sym_tick:sym_ylim);
yline(sym11_lim, Color="#5e5e5e", LineStyle="--", Label=sprintf("%d Hz", sym11_lim), LabelHorizontalAlignment='left');
legend(orbits_labels, Location="northeast");

subplot(3, 2, 6);
plot(doppler_time(1:end-sym12_n), sym12_ddiff_loop./4);
hold on;
grid on; grid minor;
xlim([doppler_time(1), doppler_time(end)]);
title(sprintf("Doppler rate, LR11xx, SF10, t_{sym} = %1.4f s", sym10_t));
xlabel("Sim. time");
xlim([dl_g dr_g]);
ylabel("Doppler rate (Hz/T_{sym})");
ylim([0, sym_ylim]);
yticks(0:sym_tick:sym_ylim);
yline(sym10_lim, Color="#5e5e5e", LineStyle="--", Label=sprintf("%d Hz", sym10_lim), LabelHorizontalAlignment='left');
legend(orbits_labels, Location="northeast");
